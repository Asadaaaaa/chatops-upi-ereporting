...
import IkuService from "../../services/Iku.service.js";
...
class Iku2Command {
  constructor(Main, ctx) {
    ...
    this.IkuService = new IkuService(this.Main);
  ...
  async startProcess(context) {
    ...
    const masterdata = await this.IkuService.getIkuData(iku_number);
    ...
    if (context.state.user.data.data_iku.formData.hasOwnProperty('dosen_id')) {
      const data = await this.IkuService.getProdiByUsername(this.username);
  ...
  async promptUserForNextInput(ctx) {
    ...
    if (state.selectableFields[nextField]) {
      ...
      const options = await this.IkuService.getProgramStudiOptions();
      const keyboard = {
        reply_markup: {
          inline_keyboard: options.map(option => [{
            text: option.nama,
      ...
      const options = await this.IkuService.getFakultasOptions();
      const keyboard = {
        reply_markup: {
          inline_keyboard: options.map(option => [{
            text: option.nama,
